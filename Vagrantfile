...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  UDP_PORTS_LIST.each do |guest, host|
    config.vm.network "forwarded_port", guest: "#{guest}", host: "#{host}", host_ip: "127.0.0.1", protocol: "udp"
  end
  
  TCP_PORTS_LIST.each do |guest, host|
    config.vm.network "forwarded_port", guest: "#{guest}", host: "#{host}", host_ip: "127.0.0.1", protocol: "tcp"
  end
